...
extension ISO_9945.Kernel.Lock.Acquire.Test.Unit {
    ...
    func `deadline case exists`() {
        let deadline = Clock.Clock.Continuous.Instant(nanoseconds: 5_000_000_000)
...
extension ISO_9945.Kernel.Lock.Acquire.Test.Unit {
    ...
    func `deadlines with same instant are equal`() {
        let instant = Clock.Clock.Continuous.Instant(nanoseconds: 10_000_000_000)
    ...
    func `deadlines with different instants are distinct`() {
        let a = ISO_9945.Kernel.Lock.Acquire.deadline(
            Clock.Clock.Continuous.Instant(nanoseconds: 1_000_000_000)
        )
        let b = ISO_9945.Kernel.Lock.Acquire.deadline(
            Clock.Clock.Continuous.Instant(nanoseconds: 2_000_000_000)
        )
        #expect(a != b)
    }
}
...
extension ISO_9945.Kernel.Lock.Acquire.Test.EdgeCase {
    ...
    func `all cases are distinct`() {
        let cases: [ISO_9945.Kernel.Lock.Acquire] = [
            .try,
            .wait,
            .deadline(Clock.Clock.Continuous.Instant(nanoseconds: 42)),
        ]
    ...
    func `deadline in the past`() {
        let pastInstant = Clock.Clock.Continuous.Instant(nanoseconds: 0)
