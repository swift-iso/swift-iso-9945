...
extension ISO_9945.Glob.Fnmatch.Test.Unit {
    ...
    func `star wildcard matches suffix`() throws {
        let matched = try Kernel.Path.scope("*.swift", "file.swift") { pattern, name in
    ...
    func `star wildcard rejects non-matching suffix`() throws {
        let matched = try Kernel.Path.scope("*.swift", "file.rs") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(!matched)
    }
    ...
    func `question mark matches single character`() throws {
        let matched = try Kernel.Path.scope("file.?", "file.c") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(matched)
    }
    ...
    func `question mark rejects multi-character`() throws {
        let matched = try Kernel.Path.scope("file.?", "file.rs") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(!matched)
    }
    ...
    func `character class matches member`() throws {
        let matched = try Kernel.Path.scope("[abc]", "a") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(matched)
    }
    ...
    func `character class rejects non-member`() throws {
        let matched = try Kernel.Path.scope("[abc]", "d") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(!matched)
    }
    ...
    func `exact literal matches`() throws {
        let matched = try Kernel.Path.scope("hello.txt", "hello.txt") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(matched)
    }
    ...
    func `exact literal rejects different name`() throws {
        let matched = try Kernel.Path.scope("hello.txt", "other.txt") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(!matched)
    }
    ...
    func `star matches empty prefix`() throws {
        let matched = try Kernel.Path.scope("*.txt", ".txt") { pattern, name in
            ISO_9945.Glob.fnmatch(pattern: pattern, name: name)
        }
        #expect(matched)
    }
...
extension ISO_9945.Glob.Fnmatch.Test.`Edge Case` {
    ...
    func `period flag prevents star from matching leading dot`() throws {
        let matched = try Kernel.Path.scope("*", ".hidden") { pattern, name in
    ...
    func `pathname flag prevents star from matching slash`() throws {
        let matched = try Kernel.Path.scope("*", "a/b") { pattern, name in
...
extension ISO_9945.Glob.Expand.Test.Unit {
    ...
    func `non-existent pattern throws noMatch`() throws {
        do throws(Path.String.Error<ISO_9945.Glob.Expand.Error>) {
            _ = try Kernel.Path.scope("/tmp/iso9945-glob-test-nonexistent-dir-*") { pattern throws(ISO_9945.Glob.Expand.Error) in
