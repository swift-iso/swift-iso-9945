...
extension ISO_9945.Kernel.File.System.Kind.Test.Unit {
    ...
    func `Kind from rawValue`() {
        let kind = ISO_9945.Kernel.File.System.Kind(rawValue: 0x1234)
        #expect(kind.rawValue == 0x1234)
    }

    @Test
    func `Kind from UInt64`() {
        let kind = ISO_9945.Kernel.File.System.Kind(0xABCD)
    ...
    func `rawValue roundtrip`() {
        ...
        let kind = ISO_9945.Kernel.File.System.Kind(rawValue: original)
...
extension ISO_9945.Kernel.File.System.Kind.Test.Unit {
    ...
    func `Kind is Sendable`() {
        let kind: any Sendable = ISO_9945.Kernel.File.System.Kind(0x1234)
        #expect(kind is ISO_9945.Kernel.File.System.Kind)
    }

    @Test
    func `Kind is Equatable`() {
        let a = ISO_9945.Kernel.File.System.Kind(0x1234)
        let b = ISO_9945.Kernel.File.System.Kind(0x1234)
        let c = ISO_9945.Kernel.File.System.Kind(0x5678)
        #expect(a == b)
        #expect(a != c)
    }
    ...
    func `Kind is RawRepresentable`() {
        let kind = ISO_9945.Kernel.File.System.Kind(rawValue: 0x1234)
        let fromRaw = ISO_9945.Kernel.File.System.Kind(rawValue: kind.rawValue)
        #expect(kind == fromRaw)
    }

    @Test
    func `Kind is CustomStringConvertible`() {
        let kind: any CustomStringConvertible = ISO_9945.Kernel.File.System.Kind(0x1234)
...
// MARK: - Linux-specific Tests
    ...
    extension ISO_9945.Kernel.File.System.Kind.Test.Unit {
        ...
        func `ext4 constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.ext4
            #expect(kind.rawValue == 0xEF53)
        }

        @Test
        func `btrfs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.btrfs
            #expect(kind.rawValue == 0x9123_683E)
        }

        @Test
        func `xfs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.xfs
            #expect(kind.rawValue == 0x5846_5342)
        }

        @Test
        func `tmpfs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.tmpfs
            #expect(kind.rawValue == 0x0102_1994)
        }

        @Test
        func `proc constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.proc
            #expect(kind.rawValue == 0x9FA0)
        }

        @Test
        func `sysfs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.sysfs
            #expect(kind.rawValue == 0x6265_6572)
        }

        @Test
        func `nfs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.nfs
            #expect(kind.rawValue == 0x6969)
        }

        @Test
        func `cifs constant exists`() {
            let kind = ISO_9945.Kernel.File.System.Kind.cifs
            #expect(kind.rawValue == 0xFF53_4D42)
        }

        @Test
        func `known filesystems have descriptive names`() {
            #expect(ISO_9945.Kernel.File.System.Kind.ext4.description == "ext4")
            #expect(ISO_9945.Kernel.File.System.Kind.btrfs.description == "btrfs")
            #expect(ISO_9945.Kernel.File.System.Kind.xfs.description == "xfs")
