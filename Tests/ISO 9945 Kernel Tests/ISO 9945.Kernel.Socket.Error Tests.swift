...
extension ISO_9945.Kernel.Socket.Error.Test.Unit {
    ...
    func `Error is Equatable`() {
        let a = ISO_9945.Kernel.Socket.Error.platform(Error.Error(code: .posix(1)))
        let b = ISO_9945.Kernel.Socket.Error.platform(Error.Error(code: .posix(1)))
        let c = ISO_9945.Kernel.Socket.Error.platform(Error.Error(code: .posix(2)))
        #expect(a == b)
        #expect(a != c)
    }
}
...
extension ISO_9945.Kernel.Socket.Error.Test.EdgeCase {
    ...
    func `Same case with different values are not equal`() {
        let a = ISO_9945.Kernel.Socket.Error.platform(Error.Error(code: .posix(1)))
        let b = ISO_9945.Kernel.Socket.Error.platform(Error.Error(code: .posix(2)))
        #expect(a != b)
    }
}
