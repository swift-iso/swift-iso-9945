...
extension Kernel.File {
    ...
    ///   - `Kernel.File.Open.Mode` - O_RDONLY, O_WRONLY, O_RDWR
    ///   - `Kernel.File.Open.Options` - O_CREAT, O_TRUNC, etc.
    ///   - `Kernel.File.Open.open()` - POSIX open() syscall
    /// - Windows: `swift-windows-primitives` (`Windows_Kernel`)
    ///   - `Kernel.File.Open.Mode` - GENERIC_READ, GENERIC_WRITE
    ///   - `Kernel.File.Open.Options` - CREATE_NEW, TRUNCATE_EXISTING, etc.
    ///   - `Kernel.File.Open.open()` - CreateFileW syscall
    ///
    /// ## See Also
